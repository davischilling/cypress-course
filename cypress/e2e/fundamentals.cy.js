// import { accordionItems } from "@/app/fundamentals/page"
...
describe('template spec', () => {
  ...
  describe('header', () => {
    it('Contains correct header text', () => {
      // passing an expression /regex/ with an "i" makes it case-insensitive
  ...
  describe('accordion', () => {
    ...
    const notVisible = (details) => {
      cy.contains(details).should('not.be.visible')
    }
  
    const clickAccordionButton = (id) => {
      // cy.get(`[data-test="accordion-item-${id}"] div[role="button"]`).click()
      cy.getDataTest(`accordion-item-${id}`, 'div[role="button"]').click()
    }

    it('Accordion works correctly', () => {
      // this works
      notVisible('Your tests will exist in a describe block')
      clickAccordionButton(1)
      visible('Your tests will exist in a describe block')
      clickAccordionButton(1)
      notVisible('Your tests will exist in a describe block')

      // this doesn't work
      // accordionItems.forEach((item) => {
      //   notVisible(item.details)
      //   clickAccordionButton(item.id)
